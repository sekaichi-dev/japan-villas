...
window.initApp = () => {
    ...
    // 0.25 Mobile Search Interactivity (Date & Guests)
    const searchModal = document.getElementById('search-modal');
    if (searchModal) {
        // Date Picker
        const dateDisplay = searchModal.querySelector('.date-display');
        ...
        const guestDisplay = searchModal.querySelector('.guest-display');
        if (guestDisplay) {
            let guestCount = 1;
            guestDisplay.style.cursor = 'pointer';
            ...
            for (let i = 1; i <= 10; i++) {
                ...
                option.innerText = isJp ? `${i}名` : `${i} Guest${i > 1 ? 's' : ''}`;
                ...
                option.addEventListener('click', () => {
                    guestCount = i;
                    const isJp = window.currentLang === 'jp';
                    guestDisplay.innerText = isJp ? `${guestCount}名` : `${guestCount} Guest${guestCount > 1 ? 's' : ''}`;
                    guestDisplay.style.color = '#fff';
                    dropdownContainer.style.display = 'none';
                });
                dropdownContainer.appendChild(option);
            }

            guestDisplay.parentElement.style.position = 'relative';
            guestDisplay.parentElement.appendChild(dropdownContainer);
            ...
            document.addEventListener('click', (e) => {
                if (!guestDisplay.contains(e.target) && !dropdownContainer.contains(e.target)) {
                    dropdownContainer.style.display = 'none';
                }
            });
        }
    ...
    if (document.body.classList.contains('home-page')) {
        ...
        if (mapContainer && typeof L !== 'undefined') {
            try {
                ...
                window.updateMapPosition = (id) => {
                    if (id === 0) {
                        map.flyTo([38.8, 138.2529], 3.85, { duration: 2.0, easeLinearity: 0.5 });
                    } else {
                        const loc = propertyLocations[id];
                        if (loc) {
            ...
            // Add observer to reset map zoom when scrolling back to original position
            if (mapContainer && window.updateMapPosition) {
                const mapObserver = new IntersectionObserver((entries) => {
                    entries.forEach(entry => {
                        // When map is entering the middle zone of viewport (scrolling back to it)
                        if (entry.isIntersecting) {
                            // Reset to show all of Japan
                            window.updateMapPosition(0);
                        }
                    });
                }, {
                    root: null,
                    threshold: 0.1, // Trigger when just 10% visible
                    rootMargin: '-40% 0px -40% 0px' // Create detection zone in middle 20% of viewport
                });
        ...
        <div onclick="openProperty(${property.id})" data-id="${property.id}" class="property-card" style="cursor: pointer;">
            ...
            <div class="card-info">
                ...
                <h3 style="margin-bottom:1rem; font-family:var(--font-display); display:flex; align-items:center;">
                    ${property.instagram ? `
                    <a href="${property.instagram}" target="_blank" class="insta-btn" onclick="event.stopPropagation()" style="margin-right: 0.5rem; margin-left: 0;">
