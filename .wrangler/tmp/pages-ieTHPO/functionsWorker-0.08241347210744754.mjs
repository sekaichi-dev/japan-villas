...
async function onRequest3(context) {
  ...
  try {
    ...
    const normalizedProperties = apiProperties.map((p) => {
      ...
      const images = Array.isArray(rawImages) ? rawImages.map((img) => img.url || img.src || img) : [];
      if (images.length === 0) images.push("images/placeholder.jpg");
      const rawAmenities = content.amenities || [];
      const features = Array.isArray(rawAmenities) ? rawAmenities.map((a) => typeof a === "object" ? a.name || a.text : a) : ["Luxury Stay"];
      return {
        id: p.id || p.propId,
...
// ../.wrangler/tmp/pages-ieTHPO/functionsRoutes-0.08397733944495023.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-EOG1Ci/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-EOG1Ci/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.08241347210744754.mjs.map
